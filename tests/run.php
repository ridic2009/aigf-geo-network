...
try {
    StudioAuth::create('writer', 'Integration-only-123!', 'author', ['us']);
    StudioAuth::create('second', 'Integration-only-123!', 'author', ['us']);
    StudioAuth::create('editor', 'Integration-only-123!', 'editor', ['us']);
    ...
    denied(fn () => StudioContent::get($author, 'fr', 'about.md'), 'site scope enforced', 403);
    denied(fn () => StudioContent::get($author, 'us', '../../config/common.yml'), 'path traversal rejected');
    denied(fn () => StudioSites::save($author, 'us', []), 'authors cannot change site configuration', 403);
    $doc = StudioContent::get($author, 'us', 'studio-test.md');
    $fm = $doc['front_matter'];
    ...
    $saved = StudioContent::save($author, 'us', 'studio-test.md', $payload);
    ...
    check(!is_file($fixture . '/content/us/studio-test.md'), 'draft does not change published source tree');
    denied(fn () => StudioContent::save($author, 'us', 'studio-test.md', $payload), 'stale revision rejected', 409);
    denied(fn () => StudioContent::save(StudioAuth::user('second'), 'us', 'studio-test.md', array_replace($payload, ['revision' => 1])), 'author cannot edit another author draft', 403);
    denied(fn () => StudioContent::transition($author, 'us', 'studio-test.md', 'approve', 1), 'author cannot approve via direct API service call', 403);
    $bad = $payload; $bad['revision'] = 1; $bad['body'] = '<script>alert(1)</script>';
    check(!StudioContent::save($author, 'us', 'studio-test.md', $bad)['ok'], 'executable content rejected');
    $missing = $payload; $missing['revision'] = 1; $missing['front_matter']['seo']['description'] = '';
    check(StudioContent::save($author, 'us', 'studio-test.md', $missing)['ok'], 'incomplete SEO can be saved as draft');
    $result = StudioContent::transition($author, 'us', 'studio-test.md', 'submit', 2);
    ...
    $payload['revision'] = 2; StudioContent::save($author, 'us', 'studio-test.md', $payload);
    StudioContent::transition($author, 'us', 'studio-test.md', 'submit', 3);
    check(StudioContent::transition($editor, 'us', 'studio-test.md', 'approve', 3)['document']['state'] === 'approved', 'editor approves specific revision');
    StudioContent::transition($editor, 'us', 'studio-test.md', 'publish', 3);
    denied(fn () => StudioContent::transition($editor, 'us', 'studio-test.md', 'publish', 3), 'duplicate publication queue entry rejected', 409);
    ...
    $after = StudioContent::get($author, 'us', 'studio-test.md');
    check($after['state'] === 'approved' && $after['published_revision'] === null, 'build-only never claims publication');
    check(!is_file($fixture . '/content/us/studio-test.md'), 'build-only leaves source tree untouched');
    $zip = new ZipArchive(); $zip->open(StudioStore::dir() . '/jobs/' . $job['id'] . '/release.zip');
    $manifest = json_decode($zip->getFromName('manifest.json'), true);
    check(isset($manifest['files']['dist/us.45.140.14.7.sslip.io/studio-test/index.html']), 'release archive contains manifest and generated page');
    $zip->close();
    $archivePath = StudioStore::dir() . '/jobs/' . $job['id'] . '/release.zip';
    $recovered = ReleaseArchive::read($archivePath, true);
    check(is_file($recovered['directory'] . '/dist/us.45.140.14.7.sslip.io/studio-test/index.html'), 'verified release can be recovered without overwriting sources');
    $tampered = StudioStore::dir() . '/tampered.zip'; copy($archivePath, $tampered);
    $zip->open($tampered); $zip->addFromString('content/us/studio-test.md', 'corrupted'); $zip->close();
    ...
    $preview = StudioPreview::create($author, 'us', 'studio-test.md');
    check(str_starts_with($preview, '/preview/') && str_ends_with($preview, '/studio-test/'), 'authenticated full-template preview generated');
    $payload['revision'] = 3; $doc = StudioContent::save($author, 'us', 'studio-test.md', $payload)['document'];
    check($doc['state'] === 'draft' && !isset($doc['approved_revision']), 'editing invalidates approval');
    denied(fn () => StudioContent::transition($editor, 'us', 'studio-test.md', 'publish', 4), 'unapproved revised draft cannot publish', 409);
    StudioContent::transition($author, 'us', 'studio-test.md', 'submit', 4); StudioContent::transition($editor, 'us', 'studio-test.md', 'approve', 4); StudioContent::transition($editor, 'us', 'studio-test.md', 'publish', 4);
    // Change after queueing: worker must cancel the old approved revision.
    $payload['revision'] = 4; StudioContent::save($author, 'us', 'studio-test.md', $payload);
    check(StudioWorker::once(false) === null, 'queued stale approval is cancelled');
    StudioContent::transition($author, 'us', 'studio-test.md', 'submit', 5); StudioContent::transition($editor, 'us', 'studio-test.md', 'approve', 5); StudioContent::transition($editor, 'us', 'studio-test.md', 'publish', 5);
    StudioAuth::update($admin, 'editor', 'author', ['us'], true);
    check(StudioWorker::once(false) === null, 'revoked publishing authority cancels pending release');
    StudioAuth::update($admin, 'editor', 'editor', ['us'], true);
    StudioContent::transition($editor, 'us', 'studio-test.md', 'publish', 5);
    file_put_contents($fixture . '/content/us/studio-test.md', 'External change');
    ...
    check(file_get_contents($fixture . '/content/us/studio-test.md') === 'External change', 'failed publication preserves external source');
    unlink($fixture . '/content/us/studio-test.md');
    ...
    check(StudioContent::get($author, 'us', 'studio-test.md')['state'] === 'publication_failed', 'next worker records interrupted publication as failed');
    denied(fn () => StudioAuth::update($admin, 'admin', 'author', ['us'], true), 'last active admin cannot be demoted');
    $created = StudioSites::save($admin, 'other-us', ['from' => 'us', 'title' => 'Other brand', 'baseurl' => 'https://other.example.com/', 'brand' => 'other', 'market' => 'us', 'language' => 'en', 'locale' => 'en_US', 'hreflang' => 'en-US', 'translation_group' => 'other', 'theme' => 'editorial', 'model' => 'reviews'], true);
    check($created['site_identity']['id'] === 'other-us' && $created['geo']['code'] === 'us', 'two brands share market without sharing site identity');
    ...
    denied(fn () => StudioSites::save($admin, 'other-us', ['baseurl' => Network::baseUrl('us')]), 'duplicate hostname rejected');
    ...
    $config = Network::geo('us'); $config['redirects'] = ['/old/' => '/middle/', '/middle/' => '/about/']; unset($config['site_identity']);
    file_put_contents(Network::configFile('us'), Yaml::dump($config, 20, 2)); Network::reset();
    check(Redirects::validate('us') === [], 'valid redirect chains accepted');
    $dist = $fixture . '/dist/redirect-test'; mkdir($dist, 0755, true); Redirects::write('us', $dist);
    check(str_contains(file_get_contents($dist . '/redirects.nginx.conf'), 'location = /old/ { return 301 /about/$is_args$args; }'), 'redirect chains collapse and preserve query string');
